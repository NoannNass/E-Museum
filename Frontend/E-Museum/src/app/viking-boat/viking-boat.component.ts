...
@Component({
  selector: 'app-viking-boat',
  imports: [],
  templateUrl: './viking-boat.component.html',
  styleUrl: './viking-boat.component.css'
})
export class VikingBoatComponent implements OnInit, OnDestroy  {
  @Input() modelPath: string = '/models/viking_ship.glb';
  @Input() modelScale: number = 1;
  @Input() modelPosition: { x: number, y: number, z: number } = { x: 0, y: 0, z: 0 };
  ...
  private initPilier() {
    const canvas = document.getElementById('pillier9') as HTMLCanvasElement;
    if (!canvas) {
  ...
  private initThreeJS() {
    const canvas = document.getElementById('model9') as HTMLCanvasElement;
    if (!canvas) {
    ...
    const loader = new GLTFLoader();
    console.log('Tentative de chargement du modèle:', this.modelPath);
    ...
    loader.load(this.modelPath, 
      ...
      (progress) => {
        console.log(`Chargement ${this.modelPath}: ${(progress.loaded / progress.total * 100).toFixed(2)}%`);
      },
      (error) => {
        console.error('Erreur de chargement du modèle:', this.modelPath, error);
      }
    );
